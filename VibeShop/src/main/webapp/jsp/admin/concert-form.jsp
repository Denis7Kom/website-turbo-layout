    ...
    <header class="admin-hero">
        <div class="admin-welcome-box">
            ...
            <div class="admin-welcome-text">
                <h1><%= editing ? "Modifica Concerto" : "Nuovo Concerto" %></h1>
                <p>Gestione dell'evento e del prezzo biglietto.</p>
            </div>
        </div>
    </header>
    ...
    <div class="admin-content-grid">
        ...
        <div class="admin-main-view">
            <section class="card-vibe admin-form-container">
                ...
                <form action="${pageContext.request.contextPath}/admin/concerts/save" method="post" class="vibe-admin-form" novalidate>
                    <input type="hidden" name="idConcerto" value="<%= editing ? concert.getIdConcerto() : 0 %>">

                    <div class="form-row">
                        <div class="form-group flex-2">
                            <label for="titoloEvento">Titolo evento</label>
                            <input type="text" id="titoloEvento" name="titoloEvento" value="<%= editing ? h(concert.getNome()) : "" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="idArtista">ID Artista</label>
                            <input type="number" id="idArtista" name="idArtista" min="1" value="<%= editing && concert.getIdArtista() != null ? concert.getIdArtista() : "" %>" />
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group flex-1">
                            ...
                            <input type="datetime-local" id="dataOra" name="dataOra" value="<%= dataOra %>" required />
                        </div>
                        <div class="form-group flex-2">
                            <label for="luogoIndirizzo">Luogo / indirizzo</label>
                            <input type="text" id="luogoIndirizzo" name="luogoIndirizzo" value="<%= editing ? h(concert.getLuogo()) : "" %>" required />
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group flex-1">
                            ...
                            <input type="number" id="prezzo" name="prezzo" step="0.01" min="0" value="<%= editing && concert.getPrezzo() != null ? concert.getPrezzo() : "" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="iva">IVA biglietto</label>
                            <input type="number" id="iva" name="iva" step="0.01" min="0" value="<%= editing && concert.getIva() != null ? concert.getIva() : "10.00" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="eventManager">Event manager</label>
                            <input type="text" id="eventManager" name="eventManager" value="<%= editing ? h(concert.getDescrizione()) : "VibeShop Events" %>" />
                        </div>
                    </div>

                    <div class="form-actions">
                        <button type="submit" class="btn-save-product">Salva Concerto</button>
                    </div>
                </form>
            </section>
        </div>
