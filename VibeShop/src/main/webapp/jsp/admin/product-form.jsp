...
<main class="admin-main-container">
    <header class="admin-hero">
        <div class="admin-welcome-box">
            <div class="admin-icon">ED</div>
            <div class="admin-welcome-text">
                <h1><%= editing ? "Modifica Prodotto" : "Nuovo Prodotto" %></h1>
                <p>Gestione del catalogo prodotti tramite Servlet e DAO.</p>
            </div>
        </div>
    </header>
                ...
                <form action="${pageContext.request.contextPath}/admin/products/save" method="post" class="vibe-admin-form">
                    ...
                    <div class="form-row">
                        <div class="form-group flex-2">
                            ...
                            <input type="text" id="nome" name="nome" value="<%= editing ? h(product.getNome()) : "" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="idArtista">ID Artista</label>
                            <input type="number" id="idArtista" name="idArtista" value="<%= editing && product.getIdArtista() != null ? product.getIdArtista() : "" %>" />
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group flex-1">
                            ...
                            <input type="number" id="prezzo" name="prezzo" step="0.01" min="0" value="<%= editing && product.getPrezzo() != null ? product.getPrezzo() : "" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="iva">IVA</label>
                            <input type="number" id="iva" name="iva" step="0.01" min="0" value="<%= editing && product.getIva() != null ? product.getIva() : "22.00" %>" required />
                        </div>
                        <div class="form-group flex-1">
                            <label for="immagine">Immagine</label>
                            <input type="text" id="immagine" name="immagine" value="<%= editing ? h(product.getImmagine()) : "" %>" />
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="descrizione">Descrizione</label>
                        <textarea id="descrizione" name="descrizione" rows="4"><%= editing ? h(product.getDescrizione()) : "" %></textarea>
                    </div>
