...
<section class="section-container">
    ...
    <div class="featured-grid"><a href="${pageContext.request.contextPath}/concerti" class="featured-card featured-card--large"><div class="featured-card-img"></div><div class="featured-card-overlay"></div><div class="featured-card-body"><span class="featured-card-label">Artista del momento</span><div class="featured-card-nome"><%= featuredArtist == null ? "Artisti VibeShop" : h(featuredArtist.getNomeArte()) %></div><div class="featured-card-genere"><%= featuredArtist == null ? "Live" : h(featuredArtist.getGenere()) %></div></div></a></div>
</section>
<section class="section-container">
    ...
    <div class="artisti-grid">
        ...
        <% } else { %>
            <% for (ArtistBean artist : artists) { %>
                <a href="${pageContext.request.contextPath}/concerti" class="artista-card"><div class="artista-card-img"></div><div class="artista-card-body"><span class="artista-card-nome"><%= h(artist.getNomeArte()) %></span><span class="artista-card-genere"><%= h(artist.getGenere()) %><%= artist.getPaese() == null ? "" : " · " + h(artist.getPaese()) %></span></div></a>
            <% } %>
        <% } %>
    </div>
</section>
